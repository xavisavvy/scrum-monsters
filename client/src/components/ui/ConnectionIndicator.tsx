...
export function ConnectionIndicator() {
  ...
  const getIndicatorProps = () => {
    // Phase 45 follow-up: bumped backgrounds from -600 to -700 so the
    // 9pt text-*-100 inside the pill clears the WCAG AA 4.5:1 contrast
    // threshold. The -600 + -100 combos were ~3:1 and tripped the
    // landing-page axe scan (e2e/a11y/lobby.a11y.spec.ts).
    if (isConnected && reconnection.status === 'connected') {
      return {
        icon: <Wifi className="w-3 h-3" />,
        title: 'Connected',
        className: 'bg-green-700 text-green-100 border-green-400',
        pulse: false
      };
    }
    ...
    switch (reconnection.status) {
      case 'reconnecting':
        return {
          ...
          className: 'bg-yellow-700 text-yellow-100 border-yellow-400',
          pulse: true
        };
      ...
      case 'failed':
        return {
          ...
          className: 'bg-red-700 text-red-100 border-red-400',
          pulse: false
        };
      ...
      default:
        return {
          ...
          className: 'bg-orange-700 text-orange-100 border-orange-400',
          pulse: false
        };
    }
  };
